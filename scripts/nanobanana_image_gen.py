#!/usr/bin/env python3
"""Generate ecommerce images with GRS AI Nano Banana 2."""
...
DEFAULT_PRODUCT = "handmade patchwork leather notebooks"
DEFAULT_SITE_USE = "global ecommerce website hero image"
DEFAULT_STYLE = (
    "beggar-inspired distressed folk craft, raw handmade leather, visible stitching, "
    "patched scraps, tactile imperfections, one-of-a-kind workshop object"
)
DEFAULT_COMPOSITION = (
    "premium product photography on a simple craft workbench, clear view of the notebooks, "
    "enough quiet negative space for website copy"
)
DEFAULT_TONE = "warm, tactile, handmade, premium but rough-edged"
DEFAULT_AVOID = (
    "watermarks, fake logos, readable text, hands with distorted fingers, plastic-looking leather, "
    "generic stock-photo staging, luxury cliches, excessive clutter, blurry products"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Generate handmade leather notebook website imagery via the GRS AI Nano Banana draw API and "
    ...
    parser.add_argument(
        ...
        help="Product subject used to build a prompt.",
    )
    parser.add_argument(
        "--site-use",
        default=DEFAULT_SITE_USE,
        help="Website asset use, such as 'hero image', 'product-card image', or 'detail close-up'.",
    )
    parser.add_argument(
        "--style",
        default=DEFAULT_STYLE,
        help="Visual style note for the product image.",
    )
    parser.add_argument(
        "--composition",
        default=DEFAULT_COMPOSITION,
        help="Composition and staging direction.",
    )
    parser.add_argument(
        "--visual-direction",
        default="",
        help="Optional extra art-direction note.",
    )
    parser.add_argument(
        "--palette",
        default="",
    ...
    parser.add_argument("--tone", default=DEFAULT_TONE, help="Overall tone for the image.")
    parser.add_argument(
        "--avoid",
        default=DEFAULT_AVOID,
        help="Things the generated image should avoid.",
    )
    parser.add_argument(
        "--reference-image",
        action="append",
    ...
    parser.add_argument(
        "--output-stem",
        default="handmade_notebook_image",
...
def build_prompt(args: argparse.Namespace) -> str:
    ...
    if args.prompt_file:
        ...
        return prompt
    if not args.product:
        raise SystemExit("Pass --product, --prompt, or --prompt-file.")

    pieces = [
        f"Create a production-ready image for a global ecommerce website selling {args.product}.",
        f"Website use: {args.site_use}.",
        f"Aspect ratio: {args.aspect_ratio}.",
        "The image must show the real product clearly enough for a shopper to understand texture, scale, and craft.",
        "Make the leather feel handmade: visible stitching, patchwork seams, worn edges, natural grain, and small imperfections.",
        f"Style: {args.style}.",
        f"Composition: {args.composition}.",
        f"Overall tone: {args.tone}.",
        "Leave useful negative space for web layout where appropriate.",
        (
            "Do not render readable words, brand names, UI, price tags, logos, numbers, labels, "
            "watermarks, or fake typography inside the image."
        ),
    ]
    ...
    pieces.append("Single clean final website image. No collage frame. No mock browser chrome.")
...
def download_file(url: str, destination: Path) -> None:
    request = urllib.request.Request(url, headers={"User-Agent": "Figurine/1.0"})
...
def main() -> int:
    ...
    manifest: dict[str, Any] = {
        "tool": "scripts/nanobanana_image_gen.py",
    ...
    if args.resume and manifest_path.exists():
        ...
        if prior_manifest.get("status") == "succeeded" and existing_files:
            print("Existing successful image run found; nothing to do.", flush=True)
